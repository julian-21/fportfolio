...
export default function ProjectsPage() {
  const projects = [
    ...
    {
      title: 'Ardatama',
      description: 'Sebuah Website Company Profile dengan Java Spring Boot backend and Vue.js frontend for real-time data processing.',
          ...
          <div className="max-w-6xl mx-auto lg:mx-0">
            ...
            <div className="space-y-16 sm:space-y-20 md:space-y-24 lg:space-y-28">
              {projects.map((project, idx) => (
                <ScrollReveal key={idx} delay={idx * 0.08} yOffset={30}>
                  <article className="group">
                    <SpotlightCard className="p-4 sm:p-6 md:p-8 border-foreground/25 bg-card/10 rounded-lg hover:border-accent/40 transition-all duration-500">
                      ...
                      <div className="space-y-3 sm:space-y-4 md:space-y-5">
                        ...
                        {/* Description */}
                        <p className="text-body text-foreground/85 leading-relaxed font-light">
                          {project.description}
                        </p>

                        {/* Full Description - Hidden on mobile, shown on larger screens */}
                        {project.fullDescription && (
                          <p className="hidden md:block text-small text-foreground/60 leading-relaxed font-light">
                            {project.fullDescription}
                          </p>
                        )}
                          ...
                          <div className="space-y-2 pt-2">
                            ...
                            <ul className="space-y-1.5 sm:space-y-2">
                              {project.features.map((feature, featureIdx) => (
                                <li 
                                  key={featureIdx}
                                  className="text-xs sm:text-sm text-foreground/75 flex items-start gap-2 font-light"
                                >
                        ...
                        <div className="flex flex-wrap gap-2 pt-3">
                          {project.tags.map((tag) => (
                            <span
                              key={tag}
                              className="text-xs font-bold uppercase tracking-wider px-2.5 sm:px-3 py-1.5 border border-foreground/30 text-foreground/70 hover:border-accent hover:text-accent hover:scale-105 transition-all duration-300 cursor-default bg-background/20"
                            >
                              {tag}
                            </span>
                          ))}
                          ...
                          <div className="pt-2">
                            <Magnet range={40} strength={0.25}>
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-2 text-sm sm:text-base font-bold text-accent hover:text-foreground transition-colors duration-300 group/link"
                              >
                                Visit Project
            ...
            {/* CTA Section */}
            <ScrollReveal className="mt-16 sm:mt-20 md:mt-24 lg:mt-28 pt-12 sm:pt-16 md:pt-20 border-t-2 border-foreground/30">
              <div className="space-y-6 sm:space-y-8 text-center lg:text-left">
                <div className="space-y-2 sm:space-y-3">
                  <h2 className="text-heading font-black text-foreground">
                    Tertarik Berkolaborasi?
                  </h2>
                  <p className="text-body text-foreground/70 max-w-xl mx-auto lg:mx-0 font-light">
